...
function Home() {
    ...
    return (
      <div className='mb-5'>
        ...
        <WelcomeMsg>Hi, <span style={{color:'red',marginRight:'11px'}}> Mehmed Al Fatih</span>👋</WelcomeMsg>
        <Container >
        <Row xs={4} className='pr-4 pl-1'>
        {books.filter((val) => searchTerm === "" || val.title.toLowerCase().includes(searchTerm.toLowerCase())
        ).map((val,index) => {
          ...
          if(index < results) {
              ...
              return <Col xs={4} key={index}>
                    ...
                    <div >
                    <Link to={`/${val.id}`} >
                      <Cover className='shadow-sm' src={val.cover}></Cover>
        ...
        <Button variant="light" className='mt-3 mb-5 ml-3' onClick={loadMore}>Load More</Button>
